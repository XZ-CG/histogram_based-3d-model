...
def Extract_point(name,pixel):#需要改写，直接抽取点进行计算histogram就行
    data = np.loadtxt(name,np.float32)
    length = data.shape[0] 
    randint = random.randint(math.ceil(length*pixel),math.ceil(length*(pixel+0.01)))
    random_data = np.zeros((randint,7),np.float32)
    for i in xrange(0,randint):
        random_data[i] = data[random.randint(0,length-1)]
    return random_data
...
def Create_input_data():
    all_file = ["f%d_final.txt"%i for i in xrange(1,21)]
    input_list = []
    for name in all_file:
        dict_all_value = Create_seq_data(name)
        input_list.append(dict_all_value)
    return input_list
...
def Read_dict(str2):
    ...
    for eachline in alllines:
        ...
        for kv in i:
            if len(kv.strip())==0:
                continue
            left,right=kv.strip().split(':')
            dict_data[left]=int(right)
        dict_result=dict(Counter(dict_result)+Counter(dict_data))
    dataset.close() 
    return dict_result    

def Create_seq_data(name):
    dict_all = Read_dict(name)
    standard = np.loadtxt('xxxxxx.txt').tolist() 
    for i in standard:
        if not dict_all.has_key(str(i)):
            dict_all[str(i)] = 0
    size = sum(dict_all.values())
    for k,v in dict_all.items():
        value = float(v)/size
        dict_all[k] = value

    dict_seq = sorted(dict_all.iteritems(),key = lambda d:d[0])
    dict_all_value = [dict_seq[i][1] for i in xrange(3125)] 
    return dict_all_value    
...
def main():
    os.chdir("C:\\Users\\lxz\\Desktop\\tmp3")
    start = datetime.now()
    foo=open('result.txt',"w")
    name={'Ida.txt':8,'Ceres.txt':1,'Chury.txt':2,'Gaspra.txt':5,'Geogra.txt':6,'Hartley.txt':7,'Steins.txt':15,'Tempel.txt':16,'Toutatis.txt':18,'Wild.txt':20,'Dione.txt':3,'Eros.txt':4,'Itokawa.txt':9,'Lutetia.txt':10,'Mimas.txt':11,'Moon.txt':12,'Phobos.txt':13,'Phoebe.txt':14,'Tethys.txt':17,'Vesta.txt':19}
    for name_asteroid,count in name.iteritems():
        internal = interval_distance(name_asteroid)
        for i in xrange(1,16):
            for time in xrange(20):
                data = Extract_point(name_asteroid,i*0.01)
                length = data.shape[0]
                col = length / 1024 + 1
                limit = 150000 / col
                row = length if length <= limit else limit
                print length,row,col*1024
                compute(count,data,0,internal,row,col,length) 
                result = third_step('f%d_0.txt'%count,load_model = 'Yes')
                print name_asteroid,result,i,time
